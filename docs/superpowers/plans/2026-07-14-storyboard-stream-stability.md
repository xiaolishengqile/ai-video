...
- [x] 为 Replay List 长度上限编写失败测试，验证 `rightPush` 后会裁剪最近 2,000 条。
- [x] 实现 Replay List 常量和裁剪调用；运行后端聚焦测试。
- [x] 停止将 `ThinkingBlock` 转换为 `REASONING` SSE 事件。
- [x] 在 pipeline store 中添加状态确认与单次重连逻辑，避免完成任务被网络异常覆盖。
- [x] 运行后端聚焦测试、前端 lint 和 build，并审查 diff。
